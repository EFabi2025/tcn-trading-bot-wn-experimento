...
class AdaptiveTCNTrainer:
    ...
    def create_definitive_tcn_model(self, input_shape: tuple):
        """
        🎯 Modelo TCN Mejorado
        - Incluye bloques residuales con dilataciones exponenciales.
        - BatchNormalization y Dropout para mejor regularización.
        - GlobalAveragePooling1D para reducir parámetros antes de Dense.
        """

        try:
        from tensorflow.keras.layers import (
            Input, Conv1D, Add, BatchNormalization, Dropout,
            Activation, GlobalAveragePooling1D, Dense
        )
        ...
        def residual_block(x, filters, dilation_rate, dropout_rate=0.2):
            ...
            # Ajustar dimensiones si cambian los filtros
        ...
        # Bloques TCN con dilataciones crecientes
        ...
        model.compile(
            ...
            loss='sparse_categorical_crossentropy',  # Cambiado para labels enteros
            metrics=['accuracy']
        )

        print(f"✅ Modelo TCN Mejorado creado: {model.count_params():,} parámetros")
        return model
